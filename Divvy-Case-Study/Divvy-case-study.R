...
```
# This analysis is based on the Case study "How Does a Bike-Share Navigate Speedy Success"

##### The data set I chose is divvy-tripdata. Here are some notes and comments about this dataset: 
* This data set contains information about trip data collected from Q2_2019 to Q1_2020
* Data set is large so I choose R to clean, analyze and visualize
...
colnames(q1_2020)
```
#### We need to rename columns  to make them consistent with q1_2020 (as this will be the supposed going-forward table design for Divvy)
...
```{r creat new data frame}
all_trips_v2 <- all_trips[!(all_trips$start_station_name == "HQ QR" | all_trips$ride_length<0),]
```

## STEP 4: CONDUCT DESCRIPTIVE ANALYSIS
#### Descriptive analysis on ride_length (all figures in seconds)
```{r decriptive analysus on ride_lenght}
...
#### I use summary() on the specific attribute to condense the four lines above to one line 
```{r summary}
...
```{r visualize for average duration}
all_trips_v2 %>% 
  mutate(weekday = wday(started_at, label = TRUE)) %>% 
  group_by(member_casual, weekday) %>% 
  summarise(number_of_rides = n()
...
#### Create a htlm & pdf file to share my findings.
